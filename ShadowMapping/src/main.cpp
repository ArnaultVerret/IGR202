...
struct Light {
  ...
  {
    depthMVP = glm::ortho(-scene_radius, scene_radius, -scene_radius, scene_radius, 0.01f, 10.f) *
    glm::lookAt(
      position, scene_center, glm::vec3(0, 1, 0)
    );
...
struct Scene {
  ...
  void render()
  {
    ...
    for(int i=0; i<lights.size(); ++i) {
      ...
      rhino->render();
      shadomMapShader->set("model", planeMat);
      plane->render();
      shadomMapShader->set("model", floorMat);
      plane->render();
    ...
    for(int i=0; i<lights.size(); ++i) {
      ...
      mainShader->set(std::string("depthTex"+std::to_string(i)), light.shadowMapTexOnGPU);
    ...
    mainShader->set("material.albedoTex", albedoTexOnGPU);
    mainShader->set("material.hasAlbedoTex", 1);
    ...
    mainShader->set("material.normalTex", normalTexOnGPU);
    plane->render();

    // floor
    mainShader->set("material.albedo", glm::vec3(0.8, 0.8, 0.9));
    mainShader->set("material.hasAlbedoTex", 0);
...
void initScene(const std::string &meshFilename)
{
  ...
  normalTex = loadTextureFromFileToGPU("data/normal.png");
  albedoTex = loadTextureFromFileToGPU("data/color.png");

  normalTexOnGPU = g_availableTextureSlot; ++g_availableTextureSlot;
  glActiveTexture(GL_TEXTURE0 + normalTexOnGPU);
  glBindTexture(GL_TEXTURE_2D, normalTex);


  albedoTexOnGPU = g_availableTextureSlot; ++g_availableTextureSlot;
  glActiveTexture(GL_TEXTURE0 + albedoTexOnGPU);
  ...
  unsigned int shadow_map_width=500, shadow_map_height=500; // play with these parameters
  for(int i=0; i<3; ++i) {
