...
public:
    ...
    const int res_x, const int res_y, const int f_width, const int f_height)
  {
    ...
    _acc = std::vector<Vec2f>(_particuleCount, Vec2f(0, 0));
...
private:
  void buildNeighbor()
  {
    ...
    // fill up the array of array of pos
    for(tIndex i=0;i<_pos.size();++i){
      tIndex x = idx1d(floor(_pos[i].x), floor(_pos[i].y));
      _pidxInGrid[x].push_back(i);
    }
  }



  std::vector<Couple> getNeighbors(int i, int j){
    bool i1=i>0, i2=i<_resX-1, j1=j>0, j2=j<_resY-1;
    std::vector<Couple> u;
    u.push_back(Couple(i, j));
    if (i1){
      if (j1){
        u.push_back(Couple(i-1, j-1));
      }
      u.push_back(Couple(i-1, j));
      if (j2){
        u.push_back(Couple(i-1, j+1));
      }
    }
    if (j1){
      u.push_back(Couple(i, j-1));
    }
    if (j2){
      u.push_back(Couple(i, j+1));
    }
  ...
  void computeDensity()
  {
    ...
    for(tIndex p = 0; p < _pos.size(); ++p){
      _d[p]=0.0f;
      int i = floor(_pos[p].x);
      int j = floor(_pos[p].y);
      // loops in the 8-convex neightbors
      for (Couple c: getNeighbors(i, j)){
        // check if it's in the gShowGrid
        for (auto &n: _pidxInGrid[idx1d(c.x, c.y)])
          _d[p] += _m0 * _kernel.w(_pos[p] - _pos[n]);
      }
    }
  }
  ...
  void computePressure()
  {
    ...
    #pragma omp parallel for
    for(tIndex p = 0; p < _pos.size(); ++p){
      _p[p]= std::max(0.0, _k*(pow(_d[p]/_d0, 7)-1));
    }
  }
  ...
  void applyBodyForce()
  {
    // loop over all particles
    #pragma omp parallel for
    for(tIndex p = 0; p < _particuleCount; ++p){
      _acc[p] += _g;
    }
  }
  ...
  void applyPressureForce()
  {
    ...
    for(tIndex p = 0; p < _particuleCount; ++p){
      ...
      for (Couple c: getNeighbors(i, j)){
        ...
        for (auto &n: _pidxInGrid[idx1d(c.x, c.y)]){
          if (n != p){
            _acc[p] -= _m0 * _kernel.grad_w(_pos[p] - _pos[n]) * (_p[p] / _d[p] / _d[p] + _p[n] / _d[n] / _d[n]);
          }
        }
      }
    }
  ...
  void applyViscousForce()
  {
    ...
    for(tIndex p = 0; p < _particuleCount; ++p){
      ...
      for (Couple c: getNeighbors(i, j)){
        ...
        for (auto &n: _pidxInGrid[idx1d(c.x, c.y)]){
          if (p != n)
            _acc[p] -= 2 * _nu * _m0 / _d[n] * (_vel[n] - _vel[p]) * _kernel.grad_w(_pos[p] - _pos[n]) * (_pos[p] - _pos[n]) / ((_pos[p] - _pos[n]) * (_pos[p] - _pos[n])+0.01*_h*_h);
        }
      }
    }
  }
  ...
  void updateColor()
  {
    for(tIndex i=0; i<_particuleCount; ++i) {
      _col[i*4+0] = 0.2;
      _col[i*4+1] = 0.2;
      _col[i*4+2] = _d[i]/_d0;
    }

    for(tIndex i=_particuleCount; i<particleCount(); ++i) {
      _col[i*4+0] = 0.2;
      _col[i*4+1] = 0.2;
      _col[i*4+2] = 0.2;
    }
...
// Executed each time the window is resized. Ah=0.5djust the aspect ratio and the rendering viewport to the current window.
...
void update(const float currentTime)
{
  if(!gAppTimerStoppedP) {
    ...
    if (gSavedCnt >= 2000){ // create 10 seconds rush
    ...
    if(gSaveFile) {
      ...
      fpath << "./img/s" << std::setw(4) << std::setfill('0') << gSavedCnt++ << ".tga";
